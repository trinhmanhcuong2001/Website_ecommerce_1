...
<form action="" method="post">
    ...
    <div class="card-body">
          ...
          <div class="form-group">
            ...
            <select name="menu_id" class="form-control">
              @foreach($menus as $menu) 
                  <option value="{{$menu->id}}" {{$product->menu_id == $menu->id ? 'seleted' : ''}}>{{$menu->name}} </option>
              @endforeach
            </select>
          </div>
        </div>
      ...
      <div class="form-group">
        ...
        <textarea name="content" class="form-control" id="ckeditor1" >{{$product->content}}</textarea>
      </div>
      <div class="form-group">
        <label for="formFile" >Ảnh sản phẩm</label>
        <input class="form-control" type="file" id="upload">
        <div id="image-show">
            <a href="{{URL::to($product->thumb)}}" target="_blank">
                <img src="{{URL::to($product->thumb)}}" width="100px" alt="">
            </a>
        </div>
      ...
      <div class="form-group">
        ...
        <div class="custom-control custom-radio">
          <input class="custom-control-input" type="radio" value="0" id="no-active" name="active" {{ $product->active == 0 ? 'checked' : ''}}>
